...
Every API key is capped at **100 requests per minute, 1,000 per hour, and 10,000 per day**, regardless of plan. Your workspace also has a burst ceiling by tier — **60 req/min on Free, 300 on Solo, 1,000 on Team** (see [pricing](/pricing)). A polite poll-and-reply loop lives well inside that, but a misbehaving one can trip it.
...
Ready to build it? Create a scoped API key and read the tool reference in [the docs](/docs), or [start free](/signup) — every plan includes unlimited tool calls, so the only thing your responder costs is the discipline to add the guardrails first.`,
};
