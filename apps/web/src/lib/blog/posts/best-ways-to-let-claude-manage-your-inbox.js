...
This is the [Model Context Protocol](https://modelcontextprotocol.io) approach, run for you. You connect your inbox once in a dashboard, paste one endpoint URL into Claude, and the agent gets a clean set of email tools. Claude calls them like any other tool: \`inbox_list\` to see your accounts, \`email_read\` to list, read, or search messages, \`email_compose\` to send, reply, or forward, plus \`email_organize\`, \`email_delete\`, \`folder\`, \`draft\`, \`schedule\`, \`signature\`, and \`contact_search\` for flags, deletions, folders, drafts, scheduling, signatures, and contacts. Ten tools in all, each picking its operation with an \`action\` parameter.
...
The Free tier is $0 forever with unlimited inboxes, 2,500 billable actions per billing period, and a 60-request/minute burst limit. Paid [plans](/pricing) raise action allowances and burst limits while adding team features. Cost is rarely the deciding factor here.
