...
每一把 API 密钥都被限制在**每分钟 100 次请求、每小时 1,000 次、每天 10,000 次**，与套餐无关。你的工作区还有一个按层级划分的突发上限——**Free 为 60 req/min，Agent 为 300，Scale 为 1,000**（见[定价](/pricing)）。一个礼貌的轮询并回复的循环会舒舒服服地待在这个范围内，但一个行为不当的循环可能会撞上去。
...
准备好动手了吗？创建一把带权限范围的 API 密钥，并在[文档](/docs)里阅读工具参考，或者[免费开始](/signup)——Free 每个计费周期含 2,500 次可计费操作；在自动化前先把护栏加好。`,
};
