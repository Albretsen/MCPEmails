...
每一把 API 密钥都被限制在**每分钟 100 次请求、每小时 1,000 次、每天 10,000 次**，与套餐无关。你的工作区还有一个按层级划分的突发上限——**免费版为 60 req/min，Pro 为 300，Team 为 1,000**（见[定价](/pricing)）。一个礼貌的轮询并回复的循环会舒舒服服地待在这个范围内，但一个行为不当的循环可能会撞上去。
...
准备好动手了吗？创建一把带权限范围的 API 密钥，并在[文档](/docs)里阅读工具参考，或者[免费开始](/signup)——免费版可连接一个收件箱，Pro 可连接你拥有的每一个邮箱；在自动化前先把护栏加好。`,
};
