...
MCP Emails 起步免费：一个已连接收件箱，API 密钥不限数量，无需信用卡。免费套餐以每分钟 60 次请求运行，配 7 天分析和社区支持。[Personal 是每月 5 美元](/pricing)（最多连接三个收件箱、120 req/min、30 天分析、邮件支持），Pro 是每月 15 美元（无限量连接收件箱、300 req/min、90 天分析），Team 是每月 79 美元（带角色的成员、每个客户一个独立工作区、SSO 和审计日志）。自托管的订阅费是 $0，外加你的 VPS 和你的工时所花的一切。对工时要诚实。
