...
两种情况各有一页摘要：个人账户见 [Outlook.com](/connect/outlook)，工作和学校账户见 [Microsoft 365](/connect/office365)。
...
在工作或学校账户上，Microsoft 可能会在同意页面出现之前拦下你，并提示需要管理员批准。出现这种情况时，控制台会给你一个可以分享的批准链接：

1. 复制链接并发给你的 IT 管理员。
2. 管理员打开链接，用其 Microsoft 管理员账户登录，并为整个组织一次性批准 MCP Emails。管理员不需要 MCP Emails 账户。
...
- 读取和搜索邮件。
- 发送、回复和转发。你发送的每封邮件最多可带 10 MB 附件，下载附件或连同附件转发原始邮件最大支持 25 MB。
- 处理草稿，并安排稍后发送。
- 管理文件夹，包括嵌套文件夹。
- 移动、复制和归档邮件。
...
**文件夹，而不是标签。** Outlook 用文件夹整理邮件。标签工具仅适用于 Gmail，因此在 Outlook 邮箱上，智能体通过把邮件移入文件夹来归类。唯一的例外是自动化规则：自动化规则里的标签操作会给邮件加上 Outlook 类别。
