...
关于速率限制，在所有套餐上，每个 API key 都被限制在每分钟 100 次请求、每小时 1,000 次、每天 10,000 次。此外还有一个按工作区计算的突发上限，会随你的套餐递增——免费版为 60/min，Pro 为 300/min，Team 为 1,000/min。当你触及上限时，服务器会返回一个可重试的错误，并附带一个以秒为单位的 \`retry_after\` 值。请遵守它。另外，绝不要盲目地自动重试 \`email_compose\` 的发送——那样你会发出重复邮件。

## 价格

套餐按连接的收件箱数量定价。API key 在所有档位都不限量，各档之间还在突发速率、分析数据保留时长、团队功能和技术支持上有差别。

- **免费版，永久 $0。** 无需信用卡。一个已连接收件箱，60 req/min，7 天分析数据，社区支持。
- **Pro，$29/month**（或 $276/year，合 $23/month）。无限量连接收件箱，300 req/min，90 天分析数据，邮件支持。
- **Team，$79/month**（或 $756/year，合 $63/month）。包含 Pro 的全部内容，另加带角色的成员、每个客户或业务一个独立工作区、支持 SAML/OIDC 的 SSO 外加审计日志，1,000 req/min，1 年分析数据，优先支持。
