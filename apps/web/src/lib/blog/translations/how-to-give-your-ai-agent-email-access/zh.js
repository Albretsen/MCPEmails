...
关于速率限制，在所有套餐上，每个 API key 都被限制在每分钟 100 次请求、每小时 1,000 次、每天 10,000 次。此外还有一个按工作区计算的突发上限，会随你的套餐递增——Free 为 60/min，Agent 为 300/min，Scale 为 1,000/min。当你触及上限时，服务器会返回一个可重试的错误，并附带一个以秒为单位的 \`retry_after\` 值。请遵守它。另外，绝不要盲目地自动重试 \`email_compose\` 的发送——那样你会发出重复邮件。

## 价格

每个套餐都不限量：邮箱、工具调用、API key、团队成员，统统不限。各档之间的差别只在突发速率、分析数据保留时长、团队功能和技术支持上。

- **Free —— 永久 $0。** 无需信用卡。60 req/min，7 天分析数据，社区支持。
- **Agent —— $12/month**（或 $120/year）。每个计费周期 50,000 次操作，300 req/min，90 天分析数据，邮件支持。
- **Scale —— $49/month**（或 $490/year）。每个计费周期 300,000 次操作，1,000 req/min，团队角色与多个工作区，支持 SAML/OIDC 的 SSO 外加审计日志，1 年分析数据，优先支持。
