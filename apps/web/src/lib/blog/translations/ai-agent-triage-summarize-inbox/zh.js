export default {
  ...
  content: `让 AI 智能体分拣收件箱最快的办法，是按顺序给它四个工具：用 \`list_inboxes\` 找到你的邮箱，用带 \`unread_only: true\` 的 \`list_messages\` 拉取新邮件，用 \`read_email\` 读那几封看起来重要的，然后给一句大白话指令让它总结并排序。整个循环就这么多。如果你想让它起草或发送回复，只需在最后加上 \`reply_to_email\`。
...
1. \`list_inboxes\` —— 发现已连接的收件箱及其 \`inbox_id\` UUID。智能体从不硬编码 UUID，而是在这里查出来。
2. 带 \`unread_only: true\` 的 \`list_messages\` —— 拉取某个收件箱的未读队列，最新的排在前面。
3. \`read_email\` —— 对那几封值得细读的邮件打开完整正文。
4. 总结并排序 —— 纯推理，不调用工具。模型负责归类、排序并给出解释。
5. \`reply_to_email\`（可选）—— 为需要回复的邮件起草或发送。
...
留意这个提示词做了什么。它让智能体倚重 \`list_messages\` 的元数据（发件人、主题、摘要片段），并尽量少调用 \`read_email\`。这能让会话保持快速，并且稳稳地待在速率限制之内——每个 API 密钥每分钟 100 个请求，以及每个工作区一个起步于免费方案 60/min 的上限。一个有 40 封未读的队列用这种方式分拣，大概是十几次工具调用，而不是四十次。
...
对于这个提示词，智能体会调用 \`read_email\` 八次，每个排名靠前的项目一次，因为你要求它别再猜。这是值得的权衡：用几次额外的调用换来你可以据此行动的排序。如果智能体在运行途中确实撞上了限制，MCP Emails 会返回一个可重试的错误，附带以秒为单位的 \`retry_after\` 值——行为良好的客户端会等够那么久再继续，而不是猛敲服务器。
...
智能体跑完只读循环，然后把草稿就地写出来。当你说「发 1 和 3」时，它会为那两封调用 \`reply_to_email\`。回复会自动归入会话线程——MCP Emails 替你设好 \`In-Reply-To\` 和 \`References\` 头，所以你的答复会落进正确的对话里，而不是另起一条新线程。邮件通过你自己的服务商发出（Gmail API、Microsoft Graph，或你的 SMTP），所以送达率和你的域名信誉始终归你所有。没有任何东西经由某个共享发送域中转。
...
这里有个老实话要说：MCP 是**轮询，不是推送。** MCP Emails 没有 webhook，也不发送任何由服务器发起的事件。新邮件到达时你的智能体不会被通知到。要做持续的分拣，就必须有某个东西按定时器去调用带 \`unread_only: true\` 的 \`list_messages\`。
...
- **点名收件箱。** 如果你连了不止一个账户，就说「我的工作收件箱」或「那个支持收件箱」。智能体会从 \`list_inboxes\` 的标题里解析出来，你也就避免了它去分拣错的邮箱。
