...
智能体会运行 \`inbox_list\`，接着用动作 \`list\` 且设置了 \`unread_only\` 的 \`email_read\`，读取那些重要的邮件，然后把一份排好序的摘要交还给我。当我看到一封想要回复的，我就跟进一句：
...
一个需要坦白的局限：没有 webhook。服务器不会主动把新邮件推送给你。如果你想让智能体对收到的邮件做出反应，就需要轮询——按计划周期性地调用动作 \`list\` 且带 \`unread_only: true\` 的 \`email_read\`。这是一个刻意的设计选择，它也决定了你如何构建任何带反应性的东西，比如一个[基于 MCP 的自动回复器](/blog/build-email-auto-responder-mcp-agent)。

## 脚本化访问的速率限制

如果你是从一个 cron 任务或一个紧密的智能体循环来驱动服务器，要留意限制。每个 API 密钥在所有套餐上都被限制为**每分钟 100 次请求、每小时 1,000 次、每天 10,000 次**。在此之上还有一个由你的套餐决定的每工作区突发上限：免费版为每分钟 60 次，Personal（每月 5 美元）为每分钟 120 次，Pro（每月 29 美元）为每分钟 300 次，Team（每月 79 美元）为每分钟 1,000 次。完整的细分见[价格页面](/pricing)。
