...
这七件事在[免费方案](/pricing)上都能跑，它是 $0，可连接一个收件箱，限速每分钟 60 个请求。Personal（每月 5 美元）最多可连接三个收件箱，并把突发上限提升到每分钟 120 个请求；Pro（每月 15 美元）可连接你拥有的每一个邮箱，并把突发上限提升到每分钟 300 个请求；Team（每月 79 美元）再加上带角色的成员、每个客户一个独立工作区、SSO，以及每分钟 1,000 个请求。一个收件箱足以支撑每周摘要；一旦你想把工作邮箱和私人邮箱放进同一个智能体，那就是每月 5 美元的 Personal；只有超过三个收件箱时才需要 Pro。
