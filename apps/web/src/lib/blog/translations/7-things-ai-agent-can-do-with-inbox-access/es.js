...
Todo lo de aquí se apoya en seis herramientas centrales: \`list_inboxes\`, \`list_messages\`, \`read_email\`, \`search_emails\`, \`send_email\` y \`reply_to_email\`. Hay unas cuantas más para marcar como leído, banderas, carpetas, envío programado y búsqueda de contactos. La primera llamada que tu agente debería hacer siempre es \`list_inboxes\`, para descubrir las cuentas conectadas y sus valores \`inbox_id\`, de modo que nunca codifiques un UUID a mano en un prompt.
...
Por debajo, el agente llama a \`list_messages\` con \`unread_only: true\` sobre la bandeja que devolvió \`list_inboxes\`, lee los pocos que parecen importantes con \`read_email\` y se salta el resto. Puede marcar como leído el ruido evidente para que deje de inflar el contador. Para una versión más a fondo de esto, el [manual de clasificación y resumen](/blog/ai-agent-triage-summarize-inbox) va herramienta por herramienta.
...
\`search_emails\` encuentra el hilo (usa la sintaxis de búsqueda nativa de tu proveedor, así que los operadores de Gmail, KQL de Outlook y la búsqueda de texto de IMAP funcionan todos), y luego \`read_email\` extrae el texto plano ya parseado de cada mensaje. \`read_email\` también puede devolver HTML saneado y adjuntos cuando lo pides, pero para un resumen el texto plano es más rápido y más barato.
...
El agente usa \`reply_to_email\`, que coloca por ti las cabeceras de hilo (In-Reply-To y References), de modo que la respuesta cae en la conversación correcta en lugar de empezar una nueva. Las respuestas admiten CC, BCC, HTML y adjuntos de hasta 10 MB en total. Mi regla: deja que el agente redacte libremente, pero mantén a una persona en el botón de enviar para cualquier cosa que salga de la empresa. Trata \`send_email\` y \`reply_to_email\` como los pasos que de verdad revisas.
...
\`search_emails\` lo acota con una consulta nativa del proveedor y luego \`read_email\` con \`include_attachments\` devuelve el archivo. Como el correo se obtiene en vivo y se descarta después de la llamada, ese adjunto no se queda en ninguna caché para más tarde. La [arquitectura sin almacenamiento](/blog/why-email-never-stored-matters) es la razón por la que es seguro apuntar un agente a esto.
...
No hay ningún evento que despierte al agente cuando llega una respuesta. En su lugar lo ejecutas según una programación —un cron, la función de tareas programadas de tu cliente, lo que encaje— y cada ejecución hace el trabajo: \`search_emails\` para las propuestas enviadas y luego \`list_messages\` para ver qué ha vuelto. Si quieres que un hilo estancado se dé un toque a sí mismo, el agente puede incluso redactar y poner en cola un seguimiento. La [construcción del autorrespondedor](/blog/build-email-auto-responder-mcp-agent) recorre el bucle de sondeo en detalle, incluido cómo respetar el valor \`retry_after\` cuando te topas con un límite de tasa para no machacar el servidor.
...
Esto cose las demás: \`search_emails\` y \`list_messages\` para reunir la semana, \`read_email\` para las que necesitan detalle y luego \`send_email\` para entregarte el resumen a ti mismo. Igual que el toque de seguimiento, es una ejecución programada, no una reacción. Tú decides la cadencia.

## Cuánto cuesta esto y dónde están los límites

Los siete funcionan en el [plan Free](/pricing), que cuesta 0 $ con bandejas y llamadas a herramientas ilimitadas. Los planes se diferencian en la tasa de ráfaga, no en lo que pueden hacer las herramientas. Free permite 60 peticiones por minuto, Solo (12 $/mes) sube a 300 y Team (49 $/mes) llega a 1.000 con roles de equipo y SSO. Un resumen semanal apenas roza esos techos; un bucle de clasificación agresivo cada minuto a través de muchas bandejas es donde notarías el límite de Free y querrías Solo.

Cuando llegues a un límite, el servidor devuelve un error reintentable con \`data.retry_after\` en segundos. Respétalo. Y nunca reintentes a ciegas una llamada a \`send_email\`, porque un reintento tras un timeout puede significar que dos copias del mismo mensaje aterricen en la bandeja de alguien.

## Empieza por una

No intentes construir las siete el primer día. Elige la pasada de clasificación o el resumen semanal, déjalo aburrido y fiable, y luego añade el siguiente. [Conecta una bandeja](/signup) y haz que tu agente ejecute \`list_inboxes\` como primer movimiento. La [referencia de herramientas en la documentación](/docs) tiene los parámetros exactos para cada llamada de arriba.`,
};
