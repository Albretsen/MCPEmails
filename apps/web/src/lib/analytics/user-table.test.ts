...
test('search reaches the fields somebody would actually paste', () => {
  const row = person({
    email: 'ana@company-07.invalid',
    primary_workspace_name: 'Company-07',
    providers: 'gmail',
    stripe_customer_id: 'cus_123',
  });
  assert.equal(matchesQuery(row, 'company-07.invalid'), true);
  assert.equal(matchesQuery(row, 'COMPANY-07'), true);
