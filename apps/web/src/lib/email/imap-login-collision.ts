 ...
 *
 * Incident context: a user connected `customer-27@redacted.invalid` (host
 * imap.domeneshop.no) but the IMAP login *username* field was autofilled with
 * `company-161` — the SASL login of a DIFFERENT mailbox (`customer-29@redacted.invalid`)
 * already connected in the same workspace. Because `company-161` + its password
 * is a VALID login, credential validation passed and the connect routes saved
 * it, so reading `customer-27@redacted.invalid` actually returned the company-16.invalid mailbox.
 *
