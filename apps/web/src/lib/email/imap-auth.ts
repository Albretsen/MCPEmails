/**
 * lib/email/imap-auth.ts — password-mechanism choice for IMAP (PLAIN, the
 * RFC 3501 LOGIN command, or RFC 2195 CRAM-MD5), plus the wire pieces of the
 * two that are not PLAIN.
 *
 * A DELIBERATE, VERBATIM COPY of supabase/functions/mcp-server/imap-auth.ts.
 * Everything from the declaration of the ImapPasswordMechanism type to the end
 * of the file is byte-identical to the edge function's copy, and "the two
 * copies of the IMAP auth module agree" (imap-auth.test.ts here, and again in
 * the Deno suite) fails the build the moment that stops being true. Why a copy
 * rather than a shared module: see the header of utf7.ts, the same situation.
 *
 * On this side it serves the connect-time check (validate-imap.ts) and the
 * dashboard's IMAP client (imap.ts). See the edge copy for the probe results
 * the mechanism rule is built on.
 */
