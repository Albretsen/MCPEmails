...
export async function runCheckout(input: {
  ...
  if (hasEntitledSubscription) {
    if (billing!.plan === planId) {
      // NOT RECORDED, and not a checkout failure. Somebody asked for the plan
      // they are already on: nothing was attempted, nothing was refused and
      // nothing could have been bought, so a `checkout_started / failure` row
      // would put a no-op click in the bucket the growth board captions "our
      // bug, not a change of mind". The two refusals that DO stay in that
      // bucket are the comped guard and the manually-managed plan above, where
      // somebody wanted to pay us and was turned away.
    ...
    try {
      ...
      if (currentItem.price.id === priceId) {
        // Not recorded either, for the same reason as `already_on_plan` above:
        // the subscription is already on the exact price being asked for.
        return fail(
          'already_on_plan_interval',
          409,
