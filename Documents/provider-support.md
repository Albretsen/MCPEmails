...
- **Signatures** are per-inbox (stored on the `inboxes` row) and appended server-side on every send, reply, forward, draft create/update, and scheduled send — so they work on **all** providers. They support **rich HTML** formatting (bold, italic, headings, lists, colors, alignment, links) and a **hosted logo/image**; the HTML is sanitized on save. Images are referenced as **hosted https URLs** (no base64 or CID) and render across every provider — some clients (e.g. Gmail) may image-block them until the recipient clicks "display images", which is expected for any hosted-image signature. On replies/forwards the signature sits after the new text and before the quoted block; `signature_reply_mode` (`always` / `first_only` (default) / `never`) controls this. The per-call `include_signature` flag on the `email_compose` and `draft` MCP tools suppresses it for a single call (default true). Signatures are managed in the dashboard per-inbox rich editor (formatted body + logo upload + enabled toggle + reply mode) and via the `signature` MCP tool, which accepts `signature_html` (`action: get` → `read:email`, `action: set` → `send:email`).
