...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `customer-16@redacted.invalid` | personal | 3 (at cap) | 2 | yahoo | **2862** | 09-07 | 07-22 | 09-01 |
| `customer-17@redacted.invalid` | personal | 3 (at cap) | 3 | gmail, icloud | 902 | 09-07 | 08-27 | 08-27 |
| `customer-21@redacted.invalid` | personal | 3 (at cap) | 1 | yandex | 844 | 09-07 | 09-05 | 09-05 |
| `customer-09@redacted.invalid` | personal | 1 | 1 | icloud | 202 | 09-07 | 08-29 | 08-29 |
| `customer-03@redacted.invalid` | personal | 2 | 2 | generic imap | 41 | 09-06 | 09-05 | 09-05 |
| `customer-18@redacted.invalid` | personal | 1 | 1 | icloud | 35 | 09-05 | 08-31 | 08-31 |
| `customer-06@redacted.invalid` | personal | 2 | 2 | generic imap | 34 | 09-04 | 09-01 | 09-01 |
| `customer-19@redacted.invalid` | **solo (Pro)** | 4 | 4 | gmail, yahoo, zoho | 29 | 09-07 | 09-07 | 09-07 |
| `customer-08@redacted.invalid` | **solo (Pro)** | **1** | 1 | generic imap | 44 | 09-05 | 08-26 | 09-05 |
| `customer-10@redacted.invalid` | **solo (Pro)** | **9** | 2 | generic imap, gmail | **2** | **09-03** | 09-02 | 09-02 |
...
| --- | --- | --- | --- |
| day +12 | `customer-24@redacted.invalid` | `customer-27@redacted.invalid` | no (grandfathered, 40,314 actions) |
| day +13 | `customer-23@redacted.invalid` | `customer-31@redacted.invalid` | no (grandfathered) |
| day +21 | `customer-13@redacted.invalid` | a second gmail | no (grandfathered) |
| day +23 | `customer-14@redacted.invalid` | `customer-30@redacted.invalid` | no (grandfathered, 637 actions) |
| day +42 | `customer-16@redacted.invalid` | `customer-26@redacted.invalid` | **yes, Personal** |
...
```
09-02 19:24:08  customer-10@redacted.invalid        (signup + first inbox)
09-02 20:02:06  proyect@company-07.invalid
09-02 20:02:52  realestate@company-07.invalid
09-02 20:05:55  facturas@company-07.invalid
09-02 20:06:30  administracion@company-07.invalid
09-02 20:07:22  mesmenjaud@company-07.invalid
09-02 20:17:48  hola@company-07.invalid
09-02 20:20:29  jcp@company-07.invalid
09-03 17:19:10  customer-25@redacted.invalid
```
...
The persona claim in [[project_first_pro_sale_company-07_20260902]] is confirmed exactly: one operator, many company mailboxes. The *expansion* framing is not. "Grew from 3 to 9 since paying" is true only in the sense that the paywall interrupted a setup session that was always going to end at 9.
...
**75% of real multi-inbox demand fits inside Personal's 3-inbox cap.** The cap is not misplaced for the median. It is misplaced for the tail, and the tail is where the money is: company-07 alone is 17% of MRR.
...
| "company-07 went 3 to 9, that is expansion" | **Refuted.** It went 1 to 9 in 56 minutes and has been flat for four days. |
| "More inboxes means more engagement" | **Not in this segment.** company-07: 9 inboxes, 2 actions. |
...
**5.1 The flagship customer does not use the product.** company-07: 9 inboxes, 2 billable actions, 8 of 9 mailboxes never synced, silent since 2026-09-03. It bought annual, so it will not visibly churn until 2027-09-02. If setup-then-abandon is the segment's pattern, the annual billing is concealing a retention problem for twelve months.
...
**5.4 Two of three Pro sales are not departmental.** `customer-08@redacted.invalid` bought unlimited inboxes while holding one. `customer-19@redacted.invalid` holds 4 inboxes on 4 different domains, which is the "each side business" persona Pro's existing copy already targets. The departmental Pro sale is n = 1.

**5.5 The Personal 3-inbox cap has fired once, ever.** One `paywall_reached` row with `category='personal'`, on 2026-09-06, from `customer-16@redacted.invalid` (3/3 inboxes, 2,862 actions, the heaviest paying user in the product). **He did not upgrade to Pro.** The single best-qualified Personal-to-Pro prospect in existence saw the offer and declined. `customer-21@redacted.invalid` is also at 3/3 with 844 actions and has not tried a fourth.
...
### 7.5 Investigate company-07's dead mailboxes. Highest urgency, smallest scope.
...
Two of the eight who started a Team checkout are `customer-10@redacted.invalid` (2026-09-02) and `customer-19@redacted.invalid` (2026-09-07). **Both then bought Pro instead, on the same day.** These are the two highest-inbox-count customers in the product. They looked at the tier whose name suggests "for a business", recoiled at $79, and stepped down to $15.

Two others were consumers with one inbox who clicked the most expensive card. One (`customer-15@redacted.invalid`) is a role address that started a Team checkout on 2026-08-27, never connected a single inbox, and never paid.
...
| 5 | Pro sales that are departmental (2+ mailboxes on ONE company domain) | 1 (company-07) | **3 or more**, moving this off n=1 | still 1 |
| 6 | company-07 reactivation: billable actions in the 30 days from 2026-09-07 | 2 all time | **50 or more** | still under 10, which makes the flagship a setup-and-abandon |
...
**Decision rule.** If metrics 1, 2 and 4 all clear their thresholds, commit to the segment: rewrite positioning, re-sort the connect waves, build `/for/business`. If 1 and 2 clear but 6 fails (company-07 stays dead), the segment converts and does not retain, and the priority becomes activation for business accounts, not more acquisition. If 4 fails while 1 clears, the segment is real but Personal absorbs it, and Pro's problem is routing, not price.
...
4. **Why did `customer-08@redacted.invalid` buy Pro with one inbox?** He paid $15/mo for unlimited mailboxes and connected one. If he bought for the 5x rate limit or the analytics rather than inbox count, then Pro's value proposition is not what its copy says, and that changes §6 materially.
