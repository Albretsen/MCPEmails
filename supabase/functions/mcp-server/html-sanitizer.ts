// ---------------------------------------------------------------------------
// html-sanitizer: dependency-free ALLOW-LIST HTML sanitizer (Deno)
// ---------------------------------------------------------------------------
//
// WHY THIS REPLACED THE REGEX DENY-LIST
//
// `sanitizeEmailHtml` and `sanitizeSignatureHtml` used to be hand-written
// chains of `.replace()` calls that hunted for known-bad shapes. That
// construction is bypassable by anything the author did not think of, and the
// following payloads all survived it verbatim in production:
//
//   <img/src="https://a/b"onerror=alert(1)>            handler after a quote,
//                                                      not after whitespace, so
//                                                      the /\s+on[a-z]+=/ rule
//                                                      never matched
//   <a href=" javascript:alert(1)">                    leading space inside the
//                                                      quoted value defeated
//                                                      /"javascript:/
//   <a href="jav&#x61;script:alert(1)">                the browser decodes the
//                                                      entity, the regex did not
//   <div style="background:url(javascript:alert(1))">  style was never inspected
//   <scr<script>ipt>alert(1)</script>                  one strip pass turns the
//                                                      payload INTO a live tag
//   <img src=https://evil.tld/pixel.gif>               unquoted value: the src
//                                                      rule only matched quoted
//                                                      ones, so every tracking
//                                                      pixel walked through the
//                                                      "no external src" policy
...
// inverted. This module tokenizes the input and REBUILDS it, emitting only
// elements and attributes that appear on an explicit allow-list, with attribute
// values re-validated after entity decoding. Anything unrecognised is dropped
// because it was never named, not because a pattern caught it.
//
// NO PARSER DEPENDENCY, ON PURPOSE
//
// The Deno edge function has no npm install step, and the Node twin
// (apps/web/src/lib/htmlSanitizer.js) must stay dependency-free because jsdom's
// ESM-only transitive deps (@asamuzakjp/css-color -> @csstools/css-calc) are
// require()d as externals under Next's server runtime and threw ERR_REQUIRE_ESM
// on every request, which 500'd every signature save in production. A sanitizer
// that cannot run is worth less than no sanitizer, so this one carries its own
// tokenizer.
//
// ONE TOKENIZER, TWO POLICIES
//
// Email bodies and signatures need different rules in exactly one place:
//
//   EMAIL      strips ALL external `src`. A body is written by a stranger, and
//              a remote image is a read receipt that fires the moment anything
//              renders it. Only `cid:` (an attachment already inside the same
//              message) and base64 raster `data:` images survive.
//   SIGNATURE  KEEPS `https:` image `src`, because a hosted logo is the entire
//              point of a rich signature, and the author is the account owner
//              rather than a stranger.
//
// That difference is DATA (see the two policy objects below), not a second copy
// of the tokenizer. Forking the code is how the two drifted apart last time.
//
// KEEP IN SYNC WITH: apps/web/src/lib/htmlSanitizer.js (Node twin, same
// algorithm, same policies, same tests). Neither can import the other: two
// runtimes, two build systems, and the shared-package alternative is not worth
// a dependency here.
//
...
/**
 * A sanitizer policy is pure data. Everything the tokenizer needs to decide
 * "keep or drop" lives here so the two call sites differ by configuration
 * rather than by implementation.
 */
export interface SanitizePolicy {
  /** Elements emitted as-is. Anything else is dropped (see `dropWithContent`). */
  allowedTags: Set<string>;
  /**
   * Elements dropped ALONG WITH their subtree. Everything else that is not in
   * `allowedTags` is *unwrapped* instead: the tag is discarded and its children
   * are kept, so an unknown wrapper loses its semantics rather than its text.
   */
  dropWithContent: Set<string>;
  /** Attributes permitted on every allowed element. */
  globalAttrs: Set<string>;
  /** Attributes permitted only on a specific element. */
  tagAttrs: Record<string, Set<string>>;
  /**
   * Schemes an `href` may carry, tested against the value AFTER entity decoding
   * and control-character removal. No match means the attribute is dropped.
   */
  hrefSchemes: RegExp;
  /**
   * Schemes an image `src` may carry, same normalisation as `hrefSchemes`.
   * One of the two axes on which the email and signature policies differ.
   */
  srcSchemes: RegExp;
  /**
   * CSS property names permitted inside a `style` attribute, or null to drop
   * `style` outright. The other axis on which the two policies differ.
   *
   * EMAIL sets this to null: a body is written by a stranger, and inline style
   * is how a phisher hides text from a human while leaving it readable to a
   * parser. SIGNATURE allows a filtered subset, because the signature editor
   * expresses ALL of its formatting as inline style and stripping it would ship
   * every rich signature as unstyled text.
   */
  styleProps: RegExp | null;
  /** Hard ceiling on the returned string. */
 ...
 * Every one of these is also in `dropWithContent`, so the raw run is discarded.
 */
 ...
 * ordinary prose. This table is deliberately NOT the full HTML5 set.
 *
 ...
 */
function normalizeUrl(raw: string, schemes: RegExp): string | null {
 ...
 */
function sanitizeStyleAttr(raw: string, props: RegExp): string | null {
  const kept: string[] = [];
...
interface ParsedTag {
  ...
  selfClosing: boolean;
  /** Index just past the `>` (or past the end of input for a truncated tag). */
  end: number;
}
 ...
 */
function parseStartTag(html: string, start: number): ParsedTag | null {
  let i = start + 1;
  const nameStart = i;
...
interface StackEntry {
  tag: string;
  /** False for unwrapped elements, whose end tag must close nothing visible. */
  emitted: boolean;
  /** True inside a `dropWithContent` subtree: tokenize on, emit nothing. */
  suppressed: boolean;
}
 ...
 * concatenation.
 */
export function sanitizeHtml(html: string, policy: SanitizePolicy): string {
...
const COMMON_TAG_ATTRS: Record<string, Set<string>> = {
  a: new Set(["href"]),
...
export const EMAIL_HTML_POLICY: SanitizePolicy = {
 ...
 *
 * It mirrors the save-time sanitizer
 * (apps/web/src/lib/sanitizeSignatureHtml.js), which is the authority on what a
 * signature may contain and has always allowed `style`, `align`, `color`,
 * `target` and `<font>`. This pass is defense-in-depth at send time, so it must
 * not be STRICTER than the thing that wrote the row: a send-time pass that
 * strips formatting the editor legitimately saved would ship every rich
 * signature as unstyled text.
 */
const SIGNATURE_TAG_ATTRS: Record<string, Set<string>> = {
  ...COMMON_TAG_ATTRS,
...
export const SIGNATURE_HTML_POLICY: SanitizePolicy = {
