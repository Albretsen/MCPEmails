...
export function destinationFolderMissingMessage(
  ...
  return (
    ...
    `operation. Nothing was moved, copied or changed — the message is exactly ` +
    `where it was. This will not succeed on a retry: the ${itemNoun} has to ` +
    `exist first. Call folder_list to see what this inbox actually has (an ` +
    `alias such as "archive" or "spam", an exact name, or an id from that list ` +
    `all resolve), then retry with one of those — or create it with ` +
    `folder { action: "create", name: ... } and retry after that.`
  );
}
