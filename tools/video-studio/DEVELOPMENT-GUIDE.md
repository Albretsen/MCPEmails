...
The tool's contract ends at `out/`. It does not write to `apps/web`. Shipping a cut is a deliberate human step, and the web side is not ready anyway. Current state, verified on `main`:

- There is **no** `apps/web/public/demo/` directory.
- `apps/web/proxy.ts` matcher excludes only `svg|png|jpg|jpeg|gif|webp|woff|woff2|ttf|otf`. It does **not** exclude `mp4|webm|mov|vtt`, so every ranged request for a video would run `updateSession()` and a Supabase auth round trip, then answer with cookies the CDN refuses to cache. That must be fixed before any video is served from `/public`.
- There is no cache header for `/demo/*` in `next.config.js`. Note that the root `vercel.json` is dead in this project, because the Vercel Root Directory is `apps/web`, so headers live in `next.config.js`.
...
There is also a **`DemoVideo.jsx` marketing component and a `scripts/demo/` toolkit sitting uncommitted** in the worktree `.claude/worktrees/brave-jackson-8180c8`. `git log --all` finds no commit touching them, so they exist only as working tree changes and are one `git checkout` away from being lost. The orchestrator should flag this to the user early. That component is already written to expect `public/demo/demo.mp4`, `poster.jpg` and `captions-en.vtt` behind a `DEMO_VIDEO_AVAILABLE` flag, so it is a good eventual target, but do not build against it until it is committed.

---

## 10. Prior work to reuse, not redo

In `.claude/worktrees/brave-jackson-8180c8/scripts/demo/` (uncommitted, see above):
...
Copy what you need into `tools/video-studio/`, or import by relative path only if those files get committed first. Do not modify them in place.
